...
class Item_ampSchema(ma.Schema):
    class Meta:
        fields = ('id', 'name', 'M1', 'M5', 'M15', 'H1', 'H4', 'D1',)
...
def post_itmes_amp():
    a = request.get_json()
    if a not in canceled_by_botton_amp:
        canceled_by_botton_amp.append(a)
    return a
...
def get_items_amp():
    all_items = Item_amp.query.all()
    result = items_schema.dump(all_items)
    result1 = items_schema.dump(all_items)
    if len(canceled_by_botton_amp) > 0:
        print(canceled_by_botton_amp)
        for canceled_by_botton_amp_i in (canceled_by_botton_amp):
            tf_cansulated = canceled_by_botton_amp_i['tf']
            curr_cansulated = canceled_by_botton_amp_i['curr']
            ind = currencys.index(curr_cansulated)
            result1[ind][tf_cansulated] = 'chay'

        for uy in range(len(canceled_by_botton_amp)):
                tf_cansulated = canceled_by_botton_amp_i['tf']
                curr_cansulated = canceled_by_botton_amp_i['curr']
                ind = currencys.index(curr_cansulated)
                if result[ind][tf_cansulated] == 'chay':
                    del canceled_by_botton_amp[uy]
    return jsonify(result1)


@app.route('/ampeng', methods=['GET'])
def get_items_eng_amp():
    all_items_eng = Ud_amp.query.all()
    result_eng = items_schema.dump(all_items_eng)
    return jsonify(result_eng)
