...
from api import db
from datetime import datetime
from api import Item_amp
from api import Ud_amp



class Datafeed(object):
    mt = r"C:\Program Files\AMP Global (EU) MT5 Terminal\terminal64.exe"
    currencys = ['EURUSD', 'GBPUSD', 'USDCAD', 'USDJPY', '@EP', '@ENQ', '@YM', '@RTY', '@DD',
                 '@DSX', '@CLE','@DB','@USA']
    ...
    def computeSMA(self, rates_frame, period):
        rates_frame['sma_tf' + str(self.TF) + '_' + str(period)] = rates_frame['close'].rolling(window=period).mean()
        return rates_frame
...
class Conditions(Datafeed):
    ...
    def con_rsi(self):
        ...
        self.cons['con_rsi14_add'] = self.df['rsi_tf' + str(self.TF) + '_14'] > 65
        ########################
        ...
        self.cons_D['con_rsi14_add'] = self.df['rsi_tf' + str(self.TF) + '_14'] < 35
        return self.cons
    ...
    def con_tendence(self):
        ...
        self.ema_cons['ema_ranked' + str(self.TF)] = (self.df['close_tf' + str(self.TF)] > self.df[
            'ema_tf' + str(self.TF) + '_' + str(20)]) & (self.df['ema_tf' + str(self.TF) + '_' + str(20)] > self.df[
        ...
        if self.TF == mt5.TIMEFRAME_M2:
            ...
            self.cons['all_trend_tfs'] = (
                    ...
                    (((self.ema_cons['con_ema20_' + str(mt5.TIMEFRAME_W1)]) &
                      (self.ema_cons['con_ema50_' + str(mt5.TIMEFRAME_W1)])) |
                     ((self.ema_cons['con_ema20_' + str(mt5.TIMEFRAME_MN1)]) &
                      (self.ema_cons['con_ema50_' + str(mt5.TIMEFRAME_MN1)])))
            )
        ...
        if self.TF == mt5.TIMEFRAME_M2:
            ...
            self.cons_D['all_trend_tfs'] = (
                    ...
                    (((self.ema_cons_D['con_ema20_' + str(mt5.TIMEFRAME_W1)]) &
                      (self.ema_cons_D['con_ema50_' + str(mt5.TIMEFRAME_W1)])) |
                     ((self.ema_cons_D['con_ema20_' + str(mt5.TIMEFRAME_MN1)]) &
                      (self.ema_cons_D['con_ema50_' + str(mt5.TIMEFRAME_MN1)])))
            )
    ...
    def con_diver(self):
        ...
        rr1 = self.cons['acc'][:rr][::-1].idxmin()
        rr2 = self.cons['acc'][:rr1][::-1].idxmax()
        ...
        rr_D = self.cons_D['acc'][::-1].idxmax()
        rr1_D = self.cons_D['acc'][:rr_D][::-1].idxmin()
        rr2_D = self.cons_D['acc'][:rr1_D][::-1].idxmax()
    ...
    def con_rsi14_up2(self):

        self.cons['con_rsi14_up2'] = (self.df['rsi_tf' + str(self.tfs[self.tfs.index(self.TF) + 2]) + '_14'] > 25) & (
                self.df['rsi_tf' + str(self.tfs[self.tfs.index(self.TF) + 2]) + '_14'] < 75)
        #########################

        self.cons_D['con_rsi14_up2'] = (self.df['rsi_tf' + str(self.tfs[self.tfs.index(self.TF) + 2]) + '_14'] < 75) & (
                self.df['rsi_tf' + str(self.tfs[self.tfs.index(self.TF) + 2]) + '_14'] > 25)
    ...
    def v_ou_j(self):
        ...
        # self.cons['rsi'] = ((self.df['stock_tf' + str(self.TF)] > 75) & (self.cons['con_rsi14_up2'] == False))
        #########################
        ...
        # self.cons_D['rsi'] = ((self.df['stock_tf' + str(self.TF)] < 25) & (self.cons_D['con_rsi14_up2'] == False))
        ...
        if self.starting_index is not None:
            ...
            for i in range(self.starting_index, len(self.cons)):
                if self.cons['cycle'].iloc[i - 1] == 'v' and self.df['stock_tf' + str(self.TF)].iloc[i] >= 25 and \
                    ...
                    self.cons['cycle'].iloc[i] = 'O'
                elif (self.cons['Oj'].iloc[i] or self.cons['O'].iloc[i]) and self.cons['cycle'].iloc[i - 1] == 'j' and \
                        self.df['close_tf' + str(self.TF)].iloc[i] > \
                        self.df['ema_tf' + str(self.TF) + '_' + str(12)].iloc[i]:
                ...
                if self.cons_D['cycle'].iloc[i - 1] == 'r' and self.df['stock_tf' + str(self.TF)].iloc[i] <= 75 and \
                    ...
                    self.cons_D['cycle'].iloc[i] = 'j'
                        ...
                        self.cons_D['init'].iloc[
                            i] == False:
                    self.cons_D['cycle'].iloc[i] = 'j'
                elif (self.cons_D['Ojr'].iloc[i] or self.cons_D['Or'].iloc[i]) and self.cons_D['cycle'].iloc[
                    i - 1] == 'r' and \
                        self.df['close_tf' + str(self.TF)].iloc[i] < \
                        self.df['ema_tf' + str(self.TF) + '_' + str(12)].iloc[i]:
...
class Eng(Datafeed):
    ...
    def eng_con_1(self, rsi, rsi_level):
        ...
        self.cons_eng['acc_prime' + '_' + str(rsi)] = (
                (self.df['rsi_tf' + str(self.TF) + '_' + str(rsi)] > rsi_level) &
                (
                        self.df['close_tf' + str(self.TF)] > self.df['open_tf' + str(self.TF)]) &
                (
                        self.df['close_tf' + str(self.TF)] > self.df['ema_tf' + str(self.TF) + '_20']) &
                (self.df['close_tf' + str(self.TF)] > self.df['ema_tf' + str(self.TF) + '_50']))

        self.cons_eng['acc_prime_2' + '_' + str(rsi)] = (
                (self.df['rsi_tf' + str(self.TF) + '_' + str(rsi)].shift(+1) > rsi_level) & (
        ...
        self.cons_eng['acc'] = (self.cons_eng['acc_prime_2' + '_' + str(rsi)]) & (
                self.df['close_tf' + str(self.TF)] < self.df['low_tf' + str(self.TF)].shift(+1)) & (
        ...
        if self.starting_index_eng is not None and self.starting_index_eng > len(self.df) - 8:
            ...
            for i in range(self.starting_index_eng, len(self.df)):
                if self.df['low_tf' + str(self.TF)][i] <= self.df['ema_tf' + str(self.TF) + '_20'][i] or \
                        self.df['close_tf' + str(self.TF)][i] > self.df['high_tf' + str(self.TF)][
                    self.starting_index_eng]:
                    self.eng_l3 = False
                    break

        ###############################

        self.cons_eng_D['acc_prime' + '_' + str(rsi)] = (self.df['rsi_tf' + str(self.TF) + '_' + str(
            rsi)] < 100 - rsi_level) & (
                                                                self.df['close_tf' + str(self.TF)] < self.df[
                                                            'ema_tf' + str(self.TF) + '_20']) & (
                                                                self.df['close_tf' + str(self.TF)] < self.df[
                                                            'ema_tf' + str(self.TF) + '_50'])

        self.cons_eng_D['acc_prime_2' + '_' + str(rsi)] = (self.df['rsi_tf' + str(self.TF) + '_' + str(rsi)].shift(
            +1) < 100 - rsi_level) & (
                                                                  self.df['close_tf' + str(self.TF)].shift(+1) <
                                                                  self.df['ema_tf' + str(self.TF) + '_20'].shift(
                                                                      +1)) & (
                                                                  self.df['close_tf' + str(self.TF)].shift(+1) <
                                                                  self.df[
                                                                      'ema_tf' + str(self.TF) + '_50'].shift(+1))

        self.cons_eng_D['acc'] = (self.cons_eng_D['acc_prime_2' + '_' + str(rsi)]) & (
                    self.df['close_tf' + str(self.TF)] > self.df['high_tf' + str(self.TF)].shift(+1)) & (
        ...
        if self.starting_index_eng_D is not None and self.starting_index_eng_D > len(self.df) - 8:
            ...
            for i in range(self.starting_index_eng_D, len(self.df)):
                if self.df['close_tf' + str(self.TF)][i] < self.df['low_tf' + str(self.TF)][
                    self.starting_index_eng_D] or \
    ...
    def eng_con_2(self):
        ...
        if flesh1 == 'U_3' and flesh2 == 'U_14':
            ...
            self.flesh = flesh2
        elif (self.cons_eng_D['acc_prime' + '_' + str(3)].iloc[-1] and
              self.cons_eng_D['acc_prime' + '_' + str(14)].iloc[-1]) or (
                self.cons_eng['acc_prime' + '_' + str(3)].iloc[-1] and self.cons_eng['acc_prime' + '_' + str(14)].iloc[
            -1]):
            self.flesh = 'G'
        return self.flesh
...
class Manage(Conditions, Eng):
    ...
    def to_db(self, df_final, df_final_eng):
        ...
        db.create_all()
        deleted_objects = Item_amp.__table__.delete()
        db.session.execute(deleted_objects)
        db.session.commit()
        deleted_objects = Ud_amp.__table__.delete()
        db.session.execute(deleted_objects)
        db.session.commit()

        for i in range(len(df_final)):
            item = Item_amp(name=self.currencys[i], M1=df_final['M2'][i], M5=df_final['M5'][i], M15=df_final['M15'][i],H1=df_final['H1'][i], H4=df_final['H4'][i], D1=df_final['D1'][i])
            db.session.add(item)
            db.session.commit()
        for i in range(len(df_final_eng)):
            ud = Ud_amp(name=self.currencys[i], M1=df_final_eng['M2'][i], M5=df_final_eng['M5'][i],
                ...
                for j in range(len(d) - 1):
                    if i != 'D1':
                        if (d[i][j] == 'O' or d[i][j] == 'Oj' or d[i][j] == 'Orsi' or d[i][j] == 'Ojrsi') and (
                                d_eng[mini[mini.index(i) + 1]][j] == 'D' or d_eng[mini[mini.index(i) + 1]][
                            j] == 'D_3' or d_eng[mini[mini.index(i) + 1]][j] == 'D_14'):
                            d[i][j] = 'X'
                            chime.warning()
                        if (d[i][j] == 'Or' or d[i][j] == 'Ojr' or d[i][j] == 'Orrsi' or d[i][j] == 'Ojrrsi') and (
                                d_eng[mini[mini.index(i) + 1]][j] == 'U' or d_eng[mini[mini.index(i) + 1]][
                            j] == 'U_3' or d_eng[mini[mini.index(i) + 1]][j] == 'U_14'):
                            d[i][j] = 'Xr'
                            chime.warning()
...
for i in mini:
    if i != 'D1':
        for j in range(len(d) - 1):
            if (d[i][j] == 'O' or d[i][j] == 'Oj' or d[i][j] == 'Orsi' or d[i][j] == 'Ojrsi' or d[i][j] == 'Osma' or
                d[i][j] == 'Ojsma' or d[i][j] == 'Orsisma' or d[i][j] == 'Ojrsisma' or d[i][j] == 'Odiv' or d[i][j] == 'Ojdiv' or d[i][j] == 'Orsidiv' or d[i][j] == 'Ojrsidiv' or d[i][j] == 'Osmadiv' or
                d[i][j] == 'Ojsmadiv' or d[i][j] == 'Orsismadiv' or d[i][j] == 'Ojrsismadiv') and (
                    d_eng[mini[mini.index(i) + 1]][j] == 'D' or d_eng[mini[mini.index(i) + 1]][j] == 'D_3' or
                    d_eng[mini[mini.index(i) + 1]][j] == 'D_14'):
                d[i][j] = 'X'
                chime.success()
            if (d[i][j] == 'Or' or d[i][j] == 'Ojr' or d[i][j] == 'Orrsi' or d[i][j] == 'Ojrrsi' or d[i][
                j] == 'Orsma' or d[i][j] == 'Ojrsma' or d[i][j] == 'Orrsisma' or d[i][j] == 'Ojrrsisma' or d[i][j] == 'Ordiv' or d[i][j] == 'Ojrdiv' or d[i][j] == 'Orrsidiv' or d[i][j] == 'Ojrrsidiv' or d[i][
                j] == 'Orsmadiv' or d[i][j] == 'Ojrsmadiv' or d[i][j] == 'Orrsismadiv' or d[i][j] == 'Ojrrsismadiv') and (
